...
REM 3DMoTraj (LBEBM3D) vs DG32-BCAT (Exp5) vs MRGTraj vs SwarmGRU v3
REM ====================================================
...
echo ============================================
echo Four-model comparison: LBEBM3D vs Exp5 vs MRGTraj vs SwarmGRU v3
echo ============================================
echo.
...
if not exist "%GRU_MODEL%" (
    echo [ERROR] SwarmGRU model not found: %GRU_MODEL%
    pause
    exit /b 1
)
...
REM Build command
set CMD=python compare_four_models.py ^
    --data_dir "%DATA_DIR%" ^
    --agents 3 ^
    --use_subset ^
