...
def main():
    ...
    all_metrics_mrgraj = []
    all_metrics_swarm = []
    all_samples_data = []  # 用于保存完整的轨迹数据以生成 4x4 图
    ...
    for i, sample_idx in enumerate(sample_indices):
        ...
        all_samples_data.append({
            ...
            'x_sample': x_sample.tolist(),             # 历史轨迹 [T_obs, A, 3]
            'y_sample': y_sample.tolist(),             # 真实未来轨迹 [T_pred, A, 3]
            'pred_mrgraj': pred_mrgraj.tolist(),
            'pred_lbebm': pred_lbebm.tolist(),
            'pred_exp5': pred_exp5.tolist(),
            'pred_swarm_gru': pred_swarm.tolist(),
            'metrics': {
                'mrgraj': metrics_mrgraj,
                'lbebm': metrics_lbebm,
                'exp5': metrics_exp5,
                'swarm_gru': metrics_swarm,
            }
        })
    ...
    summary = {
        ...
        "samples": all_samples_data  # 完整的轨迹数据用于 4x4 图生成
    }
