@echo off
REM ====================================================
REM Four-model comparison runner
REM MRGTraj vs 3DMoTraj vs VECTOR vs Ours
REM ====================================================
...
set OUTPUT_DIR=D:\Trajectory prediction\drone_trajectories\GRUTrajectoryPredictor\1k_image_comparison_four_models_results
...
set PC_CONSTRAINT_RELAXATION=1
...
REM === Inset zoom regions ===
REM Set to 0 to disable specific zoom regions in plots
set ENABLE_XY_INSET=0
set ENABLE_XZ_INSET=0
set ENABLE_YZ_INSET=0

REM ====================================================

echo ============================================
echo Four-model comparison: MRGTraj vs 3DMoTraj vs VECTOR vs Ours
echo ============================================
echo.
echo Data dir: %DATA_DIR%
echo 3DMoTraj model: %LBEBM_MODEL%
echo VECTOR dir: %EXP5_DIR%
echo MRGTraj model: %MRGRAJ_MODEL%
echo Ours model: %GRU_MODEL%
echo Output dir: %OUTPUT_DIR%
echo.

REM Validate model files
if not exist "%LBEBM_MODEL%" (
    echo [ERROR] 3DMoTraj model not found: %LBEBM_MODEL%
    pause
    exit /b 1
)

if not exist "%EXP5_DIR%\best_model_agents_3_exp5_full.pt" (
    echo [ERROR] VECTOR model not found: %EXP5_DIR%
    pause
    exit /b 1
)
...
if not exist "%GRU_MODEL%" (
    echo [ERROR] Ours model not found: %GRU_MODEL%
    pause
    exit /b 1
)
...
REM Build command
set CMD=python compare_four_models_image.py ^
    --data_dir "%DATA_DIR%" ^
    --agents 3 ^
    --use_subset ^
...
REM Inset zoom regions flags
if "%ENABLE_XY_INSET%"=="0" (
    set CMD=!CMD! --disable_insets
) else (
    if not "%ENABLE_XZ_INSET%"=="1" set CMD=!CMD! 
    if not "%ENABLE_YZ_INSET%"=="1" set CMD=!CMD! 
)
REM Note: Currently disabling insets disables all; individual control available via CLI
...
if !EXIT_CODE! equ 0 (
    ...
    echo   - sample_*_comparison_v4.png  (visualizations)
) else (
    echo ============================================
