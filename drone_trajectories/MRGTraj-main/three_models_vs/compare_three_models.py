#!/usr/bin/env python3
"""
三模型对比脚本: 3DMoTraj (LBEBM3D) vs DG32-BCAT (Exp5) vs MRGTraj
=====================================================================

对比三个高性能轨迹预测模型:
  1. LBEBM3D (3DMoTraj)         
  2. DG32-BCAT (Exp5 Full)    
  3. MRGTraj           

输出：
  - comparison_three_models/sample_xxx_comparison.png
  - comparison_three_models/comparison_summary.json
  - comparison_three_models/detailed_results.csv
"""
import argparse
import csv
import json
...
PAPER_STYLE = {
    "palette": {
        "history": "#020FFDFF",  # Light Gray - History (subtle, not blocking)
        "gt": "#07C054A7",       # Dark Green - Ground Truth (clear, distinct)
        "mrgraj": "#EC0C0C",   # Bright Red - MRGTraj (first, vibrant)
        "3dmotraj": "#C508FEFD", # Bright Blue - 3DMoTraj (second, vibrant)
        "exp5": "#00C3F9FF",     # Bright Orange - DG32-BCAT(Ours) (third, vibrant)
    },
    "linestyles": {
        "history": "-",
        "gt": "-",
    ...
    "markers": {
        "history": ".",      # Point (minimal)
        "gt": "s",           # Square
        "mrgraj": "o",       # Circle
        "3dmotraj": "^",     # Triangle Up
        "exp5": "D",         # Diamond
    },
    "linewidth": 3.0,
    "markersize": 6,
...
def load_32d_features_with_stats(features_dir, num_agents, use_subset=False):
    ...
    for path in candidates:
        if path.exists():
            ...
            data = np.load(path)
            features = data['features']  # (samples, seq, agents, 32)
...
def predict_lbebm_multi_agent(model, x_sample, device, data_scale, e_init_sig, e_prior_sig, e_l_steps, e_l_step_size, e_l_with_noise):
    ...
    for agent_idx in range(num_agents):
        ...
        try:
            ...
            # 参考脚本的实现：pred_rel[0] / data_scale + last_obs
...
def predict_exp5(model, x_sample, features, device, output_mean, output_std, feature_means, feature_stds, use_physical_constraints=True, pc_dt=0.1, pc_smoothing_weight=0.3, pc_constraint_relaxation=1.0):
    """Exp5 (DG32-BCAT) 多智能体预测 - 修复起始位置问题"""
    ...
    with torch.no_grad():
        ...
        if use_physical_constraints:
            ...
            # 确保从最后观察点开始：cumsum of deltas
            last_pos = x_sample[-1:, :, :]  # (1, num_agents, 3)
            pred_abs = last_pos + np.cumsum(pred_delta_phys, axis=0)

    # 验证起始位置正确性
    last_obs = x_sample[-1:, :, :]  # (1, num_agents, 3)
    first_pred = pred_abs[0:1, :, :]  # (1, num_agents, 3)
    distance_from_last = np.mean(np.linalg.norm(first_pred - last_obs, axis=2))
    
    if distance_from_last > 0.5:
        logger.warning(f"Exp5 预测起始点距离最后观察点 {distance_from_last:.3f}m，可能存在问题")
...
def predict_mrgraj(model, x_sample, device, use_physical_constraints=True, pc_dt=0.1, pc_smoothing_weight=0.3, pc_constraint_relaxation=1.0):
    """MRGTraj 多智能体预测 - 修复起始位置问题并添加物理约束"""
    # x_sample: (obs_len, num_agents, 3) -> (batch=1, obs_len, num_agents, 3)
    x_batch = torch.from_numpy(x_sample[np.newaxis, ...]).float().to(device)
    
    model.eval()
    with torch.no_grad():
        # MRGTrajSwarm.forward 返回 (pred_traj, mu, log_var)
        pred_traj, _, _ = model(x_batch)  # (batch, pred_len, num_agents, 3)
        pred_output = pred_traj.squeeze(0).cpu().numpy().astype(np.float32)
    
    # 检查预测是否以最后观察点为起始
    last_obs = x_sample[-1:, :, :]  # (1, num_agents, 3)
    first_pred = pred_output[0:1, :, :]  # (1, num_agents, 3)
    distance_from_last = np.mean(np.linalg.norm(first_pred - last_obs, axis=2))
    
    if distance_from_last > 1.0:  # 如果距离太远，说明可能是绝对坐标但起始不对
        logger.warning(f"MRGTraj 预测起始点距离最后观察点 {distance_from_last:.3f}m，强制对齐")
        # 计算相对位移
...
def visualize_comparison(x_sample, y_sample, pred_lbebm, pred_exp5, pred_mrgraj, sample_idx, output_path):
    """绘制三模型对比可视化 (Paper Quality - 消融实验风格)"""
    num_agents = x_sample.shape[1]
    ...
    # 创建高分辨率图表，采用 2x3 布局（参考消融实验）
    plt.rcParams['font.size'] = 11
    plt.rcParams['font.family'] = 'DejaVu Sans'
    fig = plt.figure(figsize=(28, 15), facecolor='white')
    # 使用 GridSpec 实现更好的布局控制，增加上下左右边距
    gs = fig.add_gridspec(2, 3, left=0.08, right=0.95, top=0.93, bottom=0.08, 
                         hspace=0.35, wspace=0.30)
    fig.suptitle(f'Three-Model Comparison - Sample #{sample_idx} | {num_agents} Agents', 
                fontsize=16, fontweight='bold', y=0.97)
    
    # 辅助函数：绘制带点的轨迹（消融实验风格）
    def plot_traj_with_markers(ax, data, color, linestyle, marker, label, is_3d=True, lw_val=lw, ms_val=ms):
        if is_3d:
            # 3D 轨迹：线 + 点
            ax.plot(data[:, 0], data[:, 1], data[:, 2], 
                   color=color, linestyle=linestyle, linewidth=lw_val, alpha=0.9, label=label)
            ax.scatter(data[:, 0], data[:, 1], data[:, 2], 
                      color=color, marker=marker, s=ms_val*8, alpha=0.7, edgecolors='white', linewidths=0.5, depthshade=False)
        else:
            # 2D 轨迹：线 + 点
            ax.plot(data[:, 0], data[:, 1], 
                   color=color, linestyle=linestyle, linewidth=lw_val, alpha=0.9, label=label)
            ax.scatter(data[:, 0], data[:, 1], 
                      color=color, marker=marker, s=ms_val*8, alpha=0.7, edgecolors='white', linewidths=0.5)
    ...
    for aid in range(num_agents):
        ...
        # Last observation point (connection point)
        last_pt = x_sample[-1:, aid, :]
        
        # Ground Truth (vstack with last point)
        ...
        # Model predictions
        for model_name, pred, color_key in [('MRGTraj', pred_mrgraj, 'mrgraj'), 
    ...
    bars1 = ax_bar.bar(x_pos - width, [metrics_mrgraj['ADE'], metrics_mrgraj['FDE']], width,
                      color=colors['mrgraj'], alpha=0.85, edgecolor='black', linewidth=1.5, label='MRGTraj')
    bars2 = ax_bar.bar(x_pos, [metrics_lbebm['ADE'], metrics_lbebm['FDE']], width,
                      color=colors['3dmotraj'], alpha=0.85, edgecolor='black', linewidth=1.5, label='3DMoTraj')
    bars3 = ax_bar.bar(x_pos + width, [metrics_exp5['ADE'], metrics_exp5['FDE']], width,
                      color=colors['exp5'], alpha=0.85, edgecolor='black', linewidth=1.5, label='DG32-BCAT(Ours)')
    ...
    # 保存
    png_path = str(Path(output_path).parent / f'sample_{sample_idx}_comparison_v3.png')
...
def main():
    parser = argparse.ArgumentParser(description='三模型对比: 3DMoTraj vs DG32-BCAT vs MRGTraj')
    ...
    parser.add_argument('--output_dir', default='comparison_three_models', help='输出目录')
    ...
    for i, sample_idx in enumerate(sample_indices):
        ...
        x_sample = x_all[sample_idx]  # (obs_len, num_agents, 3)
        y_sample = y_all[sample_idx]  # (pred_len, num_agents, 3)
        ...
        if not args.no_visualize:
            ...
            visualize_comparison(x_sample, y_sample, pred_lbebm, pred_exp5, pred_mrgraj, sample_idx, str(viz_path))
    ...
    # 性能对比
    logger.info("\n=== 性能改进 ===")
    if agg_lbebm['ADE']['mean'] > 0:
        exp5_vs_lbebm = (agg_lbebm['ADE']['mean'] - agg_exp5['ADE']['mean']) / agg_lbebm['ADE']['mean'] * 100
        mrgraj_vs_lbebm = (agg_lbebm['ADE']['mean'] - agg_mrgraj['ADE']['mean']) / agg_lbebm['ADE']['mean'] * 100
        mrgraj_vs_exp5 = (agg_exp5['ADE']['mean'] - agg_mrgraj['ADE']['mean']) / agg_exp5['ADE']['mean'] * 100

        logger.info(f"Exp5 vs LBEBM3D (ADE):      {exp5_vs_lbebm:+.2f}%")
        logger.info(f"MRGTraj vs LBEBM3D (ADE):   {mrgraj_vs_lbebm:+.2f}%")
        logger.info(f"MRGTraj vs Exp5 (ADE):      {mrgraj_vs_exp5:+.2f}%")
    ...
    with open(csv_summary_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for model_name, agg in [("LBEBM3D", agg_lbebm), ("DG32-BCAT", agg_exp5), ("MRGTraj", agg_mrgraj)]:
    ...
    with open(csv_samples_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for idx, (m_lbebm, m_exp5, m_mrgraj) in zip(sample_indices, zip(all_metrics_lbebm, all_metrics_exp5, all_metrics_mrgraj)):
            for name, metrics in (
                ("LBEBM3D", m_lbebm),
                ("DG32-BCAT", m_exp5),
                ("MRGTraj", m_mrgraj),
