...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        if x.dim() != 3:
            raise ValueError(f"PositionalEncoding expects 3D tensor, got {x.dim()}D")

        # 判断输入是 batch-first 还是 seq-first
        # 常见情况: batch-first -> (B, S, D); seq-first -> (S, B, D)
        if x.size(0) <= x.size(1):
            # seq-first: (S, B, D)
            seq_len = x.size(0)
            pe = self.pe[:, :seq_len, :].transpose(0, 1)  # (S, 1, D)
        else:
            # batch-first: (B, S, D)
            seq_len = x.size(1)
