...
def parse_args():
    ...
    parser.add_argument('--gpu_deterministic', action='store_true', help='set cudnn in deterministic mode (slow)')
    ...
    parser.add_argument("--sub_goal_indexes", default=[2, 5, 7, 9], type=list)
    ...
    parser.add_argument('--e_l_with_noise', action='store_true', help='noise term of langevin')
    parser.add_argument('--e_sn', action='store_true', help='spectral regularization')
    parser.add_argument('--e_lr', default=0.00003, type=float)
    parser.add_argument('--e_is_grad_clamp', action='store_true', help='whether doing the gradient clamp')
    ...
    parser.add_argument('--save_dir', type=str, default='', help='directory to save checkpoints (best/last/epoch_xxx)')
    parser.add_argument('--save_every', type=int, default=0, help='save every N epochs (0 to disable)')
    ...
    parser.add_argument('--vis', action='store_true', help='whether visualize predicted results')
...
def main():
    ...
    trajaugmenter = TrajectoryAugmenter(args.obs + args.preds, args.obs)
    ...
    def test(model, dataloader, dataset, sub_goal_indexes, best_of_n=20, return_predictions=False):
        model.eval()
        ...
        all_ground_truth = []  # 保存所有真实值
        all_input_history = []  # 保存所有输入历史
        ...
        for i, trajx in enumerate(dataloader):
            ...
            if return_predictions:
                x_np = x.cpu().numpy().reshape(-1, args.past_length, 3)
                all_input_history.append(x_np)
                all_ground_truth.append(y)
                all_predictions.append(predicted_future)

        total_overall_err /= len(dataset)
        total_dest_err /= len(dataset)

        if return_predictions:
            return total_overall_err, total_dest_err, np.concatenate(all_input_history), np.concatenate(all_ground_truth), np.concatenate(all_predictions)
        else:
    ...
    def run_training(args):
        ...
        def checkpoint_dir():
            ...
            # fallback: use model_path directory
            return os.path.dirname(args.model_path) or "."
        ...
        # ----- resume -----
        if args.resume:
            ckpt_resume = load_resume(args.resume)
            if ckpt_resume:
                ...
                best_val_ade = ckpt_resume.get('best_val_ade', best_val_ade)
                best_val_fde = ckpt_resume.get('best_val_fde', best_val_fde)
                print(f"Resumed from {args.resume}, start_epoch={start_epoch}, best_val_ade={best_val_ade:.4f}")
        ...
        for epoch in range(start_epoch, args.num_epochs):
            ...
            if best_val_ade > overall_err:
                ...
                save_checkpoint({
						...
						}, 'best.pt' if args.save_dir else os.path.basename(args.model_path))
                print("Saved best model to:\n{}".format(os.path.join(checkpoint_dir(), 'best.pt' if args.save_dir else os.path.basename(args.model_path))))
    ...
    def run_eval(args):
        ...
        # 获取预测结果
        overall_err, dest_err, input_history, ground_truth, predictions = test(
            model, test_dl, test_dataset, args.sub_goal_indexes, args.n_values, return_predictions=True
        )
        logger.info("{}, test ADE {}".format(args.dataset_name, overall_err))
        logger.info("{}, test FDE {}".format(args.dataset_name, dest_err))
        
        # 保存预测结果到pickle文件
        output_file = os.path.join(output_dir, 'predictions.pickle')
        predictions_data = {
            'input_history': input_history,      # (N, 20, 3) - 输入历史
            'ground_truth': ground_truth,        # (N, 10, 3) - 真实未来
            'predictions': predictions,          # (N, 10, 3) - 预测未来
            'ade': overall_err,
            'fde': dest_err,
            'args': args
        }
