# modified from https://github.com/FGiuliari/Trajectory-Transformer/blob/master/baselineUtils.py
...
def create_dataset(dataset_folder,dataset_name,val_size,gt,horizon,delim="\t",train=True,eval=False, rotate=False, recreate=False, verbose=False):
        ...
        datasets_list = [f for f in datasets_list if 'pickle' not in f]
        ...
        saved_data = os.path.join(full_dt_folder, 'saved_data' + ('_rotate_' if rotate else '') + '.pickle')
        if os.path.exists(saved_data) and not recreate:
            with open(saved_data, 'rb') as f:
                data = pickle.load(f)
            print('loaded preprocessed data file {}'.format(saved_data))
        else:
            for i_dt, dt in enumerate(datasets_list):
                if verbose:
                    print("%03i / %03i - loading %s"%(i_dt+1,len(datasets_list),dt))
                ped_dataset = ['sdd', 'eth', 'univ', 'hotel', 'zara1', 'zara2']
                if dataset_name in ped_dataset:
                    raw_data = pd.read_csv(os.path.join(full_dt_folder, dt), delimiter=delim, names=["frame", "ped", "x", "y"],usecols=[0,1,2,3],na_values="?")
                else:
                    raw_data = pd.read_csv(os.path.join(full_dt_folder, dt), delimiter=delim, names=["frame", "ped", "x", "y", "z"],usecols=[0,1,2,3,4],na_values="?")
                raw_data.sort_values(by=['frame','ped'], inplace=True)

                if dataset_name in ped_dataset:
                    inp,out,info=get_strided_data_clust(raw_data, gt, horizon, 20, rotate=rotate)
                else:
                    inp,out,info=get_strided_3Ddata_clust(raw_data, gt, horizon, 20, rotate=rotate)
...
def get_strided_data_clust(dt, gt_size, horizon, step, rotate=False):
    inp_te = []
    dtt = dt.astype(np.float32)
    raw_data = dtt
    ...
    frame=[]
    ped_ids=[]
    # angles = np.arange(0, 360, 15) if rotate else [0]
    angles = [0, 0.25 * np.pi, 0.5 * np.pi, -0.25 * np.pi, -0.5 * np.pi] if rotate else [0]
    for p in ped:
        for i in range(1+(raw_data[raw_data.ped == p].shape[0] - gt_size - horizon) // step):
            # frame.append(dt[dt.ped == p].iloc[i * step:i * step + gt_size + horizon, [0]].values.squeeze())
            # inp_te.append(raw_data[raw_data.ped == p].iloc[i * step:i * step + gt_size + horizon, 2:4].values)
            ...
            frame.append(dt[dt.ped == p].iloc[i * step:i * step + gt_size + horizon, [0]].values.squeeze())
            ped_ids.append(p)
            xy = raw_data[raw_data.ped == p].iloc[i * step:i * step + gt_size + horizon, 2:4].values
            # xy = xy @ rotation_matrix(angle)
            inp_te.append(xy)
    ...
    ped_ids_aug = []
    for angle in angles:
        ped_ids_aug.append(np.matmul(inp_te_np, rotation_matrix(angle)))

    inp_te_np = np.concatenate(ped_ids_aug) 
    frames = np.tile(frames, (len(angles), 1))
    ped_ids = np.tile(ped_ids, (len(angles), ))
...
def get_strided_3Ddata_clust(dt, gt_size, horizon, step, rotate=False):
    inp_te = []
    dtt = dt.astype(np.float32)
    raw_data = dtt
    
    # Define 3D rotation matrices for different axes
    ...
    for p in ped:
        for i in range(1+(raw_data[raw_data.ped == p].shape[0] - gt_size - horizon) // step):
            frame.append(dt[dt.ped == p].iloc[i * step:i * step + gt_size + horizon, [0]].values.squeeze())
            ped_ids.append(p)
            xyz = raw_data[raw_data.ped == p].iloc[i * step:i * step + gt_size + horizon, 2:5].values
            inp_te.append(xyz)
    if len(frame) != 0:
        frames = np.stack(frame)
        inp_te_np = np.stack(inp_te)
    ...
    if len(angles) != 1:
        angle_id = random.randint(1, len(angles)-1)
        angles = [0, angles[angle_id]]

    for angle in angles:
        axes = [rotation_matrix_x, rotation_matrix_y, rotation_matrix_z]
        axis_id = random.randint(0, len(axes)-1)
        ped_ids_aug.append(np.matmul(inp_te_np, axes[axis_id](angle)))


        
    inp_te_np = np.concatenate(ped_ids_aug)
    #frames = np.tile(frames, (len(angles) * 3, 1))
    #ped_ids = np.tile(ped_ids, (len(angles) * 3,))
    frames = np.tile(frames, (len(angles), 1))
    ped_ids = np.tile(ped_ids, (len(angles), ))

    inp_speed = np.concatenate((np.zeros((inp_te_np.shape[0], 1, 3)), inp_te_np[:, 1:, 0:3] - inp_te_np[:, :-1, 0:3]), 1)
    inp_norm = np.concatenate((inp_te_np, inp_speed), 2)
    inp_mean = np.zeros(6)
    inp_std = np.ones(6)
