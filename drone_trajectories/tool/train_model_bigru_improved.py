...
class EnhancedGRUModel(nn.Module):
    """
    BiGRU + Cross-Attention 模型
    核心特点：
    - BiGRU编码器：bidirectional=True，保留所有位置的前向和反向信息
    - Cross-Attention解码器：每个解码步都能访问编码器的全部输出
    - 充分利用双向编码的优势，避免信息浪费
    """
    def __init__(self, input_size=16, hidden_dim=128, num_layers=3, 
                 dropout=0.5, output_steps=10, use_attention=True, bidirectional=True,
        ...
        Args:
            ...
            dropout: dropout 概率
            output_steps: 输出步数
            use_attention: BiGRU必须启用Cross-Attention (始终True)
            bidirectional: 编码器必须为双向 (始终True)
            encoder_input_dim: encoder_gru 的输入维度
        """
        super().__init__()
        ...
        # ✓ BiGRU特性：必须使用Cross-Attention充分利用双向信息
        ...
        # ✓ BiGRU编码器：bidirectional=True，处理完整序列获得前后向信息
        self.encoder_gru = nn.GRU(
            ...
            bidirectional=True,  # ← 关键：必须双向
        )
        
        # ✓ Cross-Attention机制：让解码器在每步都访问编码器的全部输出
        num_heads = 1
        ...
        # ✓ 关键创新：Cross-Attention解码
        # Query来自解码器隐藏状态，Key和Value来自编码器全部输出
    ...
    def forward(self, x, return_plane_preds: bool = False):
        ...
        # ✓ BiGRU编码：encoder_out包含所有位置的前向和反向信息
        enc_out, h = self.encoder_gru(x_fused)  # enc_out: (B, T, hidden_dim*2)
        h = self._merge_bidirectional_hidden(h)

        # ✓ 编码器精化：Self-Attention在编码器输出上做进一步融合
        enc_out = self.pos_enc(enc_out)
        enc_out = self.enc_refiner(enc_out)

        # ✓ 自回归解码：每步都通过Cross-Attention查询全部编码器输出
        predictions = []
        plane_buffers = {plane: [] for plane in self.plane_heads.keys()}
        h_t = h

        # 初始化：用Cross-Attention获取第一步的上下文
        q0 = enc_out[:, -1:, :]
        ctx0, _ = self.cross_attn(q0, enc_out, enc_out, need_weights=False)
        ctx0 = self.cross_ln(q0 + ctx0).squeeze(1)
        prev_output = self.fc(ctx0)
        ...
        for _ in range(self.output_steps):
            ...
            h_last = h_t[-1]  # 解码器隐藏状态作为Query

            # ✓ 关键：Cross-Attention让Query查询编码器的所有位置
            # Query: 当前解码步的隐藏状态 (B, 1, H)
            # Key/Value: 编码器全部输出 (B, T, H) ← 充分利用双向信息
            q = h_last.unsqueeze(1)
            ctx, _ = self.cross_attn(q, enc_out, enc_out, need_weights=False)
            ctx = self.cross_ln(q + ctx).squeeze(1)
            
            base_output = self.fc(ctx)
            plane_preds_step = self._compute_plane_predictions(ctx)
...
def model_forward_with_adaptive_tf(model, input_seq, target_seq, teacher_forcing_ratio=0.5,
    ...
    """
    BiGRU + Cross-Attention 自适应Teacher Forcing前向传播
    """
...
def main():
    ...
    # ✓ BiGRU模型必须使用Cross-Attention充分利用双向编码
    model = EnhancedGRUModel(
        ...
        output_steps=10,
        use_attention=True  # ← 强制启用Cross-Attention
    )
    model.to(device)
    
    logger.info(f"\n模型配置:")
    logger.info(f"  架构: BiGRU + Cross-Attention (充分利用双向编码)")
    ...
    logger.info(f"  编码器: BiGRU (bidirectional=True)")
    logger.info(f"  解码器: Cross-Attention (查询全部编码器输出)")
    ...
    config_dict = {
        ...
        'use_attention': True,  # BiGRU必须使用Cross-Attention
        'bidirectional': True,  # BiGRU必须为双向
        'model_type': 'BiGRU + Cross-Attention',
