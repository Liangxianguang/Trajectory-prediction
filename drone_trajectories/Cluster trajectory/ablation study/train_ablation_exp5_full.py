...
def main():
    parser = argparse.ArgumentParser(description='消融实验5：完整模型 DG32-BCAT (32D, GAT+特征增强+BiGRU+CA)')
    ...
    data_info = load_ablation_data(
        args.data_dir, args.agents, feature_dim=32,
    ...
    logger.info("创建完整模型 DG32-BCAT...")
    # ✅ 直接使用经过验证的v3模型类，只需设置input_size=32（就像v4那样）
    model = DynamicsAwareSwarmGRUModel_with_GNN(
        input_size=32,  # 32D特征（v3原本是24D，v4就是这样做的）
    ...
    logger.info(f"✓ 模型创建: 完整模型 DG32-BCAT (GAT+特征增强+BiGRU+Cross Attention)")
    logger.info(f"  ✅ 使用经过验证的v3模型类 (DynamicsAwareSwarmGRUModel_with_GNN)")
    logger.info(f"  ✅ 与v4使用相同的模型架构（v4就是这样做的）")
    ...
    suffix = f"agents_{args.agents}_exp5_full"
    ...
    config = {
        ...
        'description': '完整模型 DG32-BCAT (32D, GAT+特征增强+BiGRU+Cross Attention)',
        'input_features': 32,
