...
# -*- coding: utf-8 -*-
"""
消融实验4：GAT + 特征增强（无BiGRU+Cross Attention）
配置：√ √ × (32D)
- GAT
- 特征增强（32D特征）
...
from ablation_models import GNNFeatureModel
...
def main():
    parser = argparse.ArgumentParser(description='消融实验4：GAT+特征增强 (32D, 无BiGRU+CA)')
    ...
    model = GNNFeatureModel(
        ...
        output_size=3,
        dropout=0.3,
        use_attention=False,  # 无Cross Attention
    ...
    logger.info(f"✓ 模型创建: GAT+特征增强（单向GRU，无Cross Attention）")
    ...
    suffix = f"agents_{args.agents}_exp4_gnn_feat"
    ...
    config = {
        ...
        'experiment': 'exp4_gnn_feat',
        'description': 'GAT+特征增强 (32D, 无BiGRU+Cross Attention)',
        'input_features': 32,
