...
def load_swarm_data(data_dir, num_agents, batch_size=32, val_split=0.2, 
                   features_dir=None, num_workers=4, prefetch_factor=2, use_subset=False):
    """
    加载数据并计算特征统计量（参考单机模型 train_model_bigru_improved.py）
    返回: train_loader, val_loader, 统计量字典
    
    Args:
        use_subset: 如果为True，加载 input_agents_{num_agents}_subset.npz
    """
    data_path = Path(data_dir)
    
    # 根据 use_subset 标志选择文件
    ...
    if features_dir is not None:
        # 根据 use_subset 标志选择特征文件
        subset_suffix = '_subset' if use_subset else ''
        features_path = Path(features_dir) / f'features_agents_{num_agents}{subset_suffix}.npz'
...
def main():
    ...
    parser.add_argument('--use_subset', action='store_true',
                        help='使用 _subset.npz 子集数据 (用于快速测试和调试)')
    ...
    for num_agents in agents_list:
        ...
        try:
            train_loader, val_loader, stats = load_swarm_data(
                ...
                prefetch_factor=args.prefetch_factor, use_subset=args.use_subset
            )
            # 解包统计量
        ...
        if args.resume:
            ...
            try:
                ...
                logger.warning(f"⚠ 检查点加载失败（可能是模型结构不匹配）: {e}")
                logger.warning(f"⚠ 忽略此检查点，从头开始训练")
                logger.info(f"提示：使用 --no_resume 跳过自动恢复")
