...
def load_pos_stats(filename):
    with np.load(filename, allow_pickle=True) as data:
        ...
        # L_matrix may not exist in all stat files
        if 'L_matrix' in data:
            L_matrix = data['L_matrix']
        else:
            # Create identity matrix if L_matrix is missing
            L_matrix = np.eye(3)
        
        # max_length is optional
        max_length = data.get('max_length', None)
        if max_length is None and 'pos_max' in data:
            max_length = data['pos_max']
...
def load_vel_stats(filename):
    with np.load(filename, allow_pickle=True) as data:
        ...
        # max_velocity is optional
        max_velocity = data.get('max_velocity', None)
        if max_velocity is None and 'vel_max' in data:
            max_velocity = data['vel_max']
...
def whiten_3d_sequence(sequence, mean, L):
    ...
    m = np.asarray(mean)
    # If mean is time-varying (shape (T,3)) but doesn't match sequence length,
    # collapse it to a single mean vector.
    if m.ndim == 2:
        if m.shape[0] == seq.shape[0]:
            mean_used = m
        else:
            mean_used = m.mean(axis=0)
    else:
        mean_used = m

    whitened_sequence = np.dot(seq - mean_used, L.T)
    return whitened_sequence

def dewhiten_3d_sequence(whitened_sequence, mean, L):
    # Assuming sequence of shape (N,3)
    # Apply dewhitening transformation
    seq = np.asarray(whitened_sequence)
    m = np.asarray(mean)
    # Decide mean to add back. If mean is time-varying but its length doesn't
    # match prediction length, use its time-average as a vector.
    if m.ndim == 2:
        if m.shape[0] == seq.shape[0]:
            mean_used = m
        else:
    ...
    dewhitened_sequence = np.dot(seq, L_inv.T) + mean_used
